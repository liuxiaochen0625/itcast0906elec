...
<link href="${pageContext.request.contextPath }/css/Style.css" type="text/css" rel="stylesheet">
  ...
  function checkchar(){
	  ...
	  if(document.Form1.stationRun.value.length>2500){
	  
	     alert("站点运行情况字数不能超过2500字");
	     return;
	  }
	  if(document.Form1.devRun.value.length>2500){
  ...
  //添加进度条
	...
	function openContenFrame(){
	    ...
	    setTimeout('openContenFrame()',100) ;//每隔0.1秒执行openContenFrame()
	}
	function loading(){
	    document.getElementById("load").style.display="";//显示
	    document.getElementById("opperate1").style.display="none";//隐藏
	    document.getElementById("opperate2").style.display="none";//隐藏
	    openContenFrame();
	}
  </script>
